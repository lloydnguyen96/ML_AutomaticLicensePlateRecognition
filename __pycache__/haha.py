...
def do_contain_other_possible_plates(boundingRectElement, boundingRectInformation):
	x = boundingRectElement[0]
	y = boundingRectElement[1]
	w = boundingRectElement[2]
	h = boundingRectElement[3]
	print('boundingRectElement')
	print(boundingRectElement)
	if len(boundingRectInformation) == 0:
		print('lau dau')
		return False
	else:
		for plateRect in boundingRectInformation:
			print('plateRect')
			print(plateRect)
			plateRectX = plateRect[0]
			plateRectY = plateRect[1]
			plateRectW = plateRect[2]
			plateRectH = plateRect[3]
			if plateRectX > x and plateRectY > y and plateRectX + plateRectW < x + w and plateRectY + plateRectH < y + h:
				return True
	return False

# Constants: 800, 650
IMAGE_SCENE_WIDTH = 600
IMAGE_SCENE_HEIGHT = 500
...
# min = 80
imgScene = cv2.imread('4.jpg')
imgSceneHeight, imgSceneWidth, channel = imgScene.shape
imgSceneAspectRatio = float(imgSceneWidth)/imgSceneHeight
imgSceneArea = imgSceneWidth * imgSceneHeight
print('imgSceneWidth: ' + str(imgSceneWidth))
print('imgSceneHeight: ' + str(imgSceneHeight))
cv2.imshow('imgScene', imgScene)


# Step 2: Resize large images
imgSceneResized = imgScene.copy()
imgSceneResizedHeight, imgSceneResizedWidth, channelResized = imgSceneResized.shape
if imgSceneResizedWidth > IMAGE_SCENE_WIDTH:
	imgSceneResized = cv2.resize(imgSceneResized, (IMAGE_SCENE_WIDTH, int((IMAGE_SCENE_WIDTH) * (imgSceneAspectRatio)**(-1))), interpolation = cv2.INTER_AREA)
	imgSceneResizedHeight, imgSceneResizedWidth, channel = imgSceneResized.shape
if imgSceneResizedHeight > IMAGE_SCENE_HEIGHT:
	imgSceneResized = cv2.resize(imgSceneResized, (int(IMAGE_SCENE_HEIGHT * (imgSceneAspectRatio)), IMAGE_SCENE_HEIGHT), interpolation = cv2.INTER_AREA)
	imgSceneResizedHeight, imgSceneResizedWidth, channel = imgSceneResized.shape
resizeRatio = float(imgSceneWidth)/imgSceneResizedWidth
print('resizeRatio: ' + str(resizeRatio))
imgSceneResizedArea = imgSceneResizedWidth * imgSceneResizedHeight
print('imgSceneResizedWidth: ' + str(imgSceneResizedWidth))
print('imgSceneResizedHeight: ' + str(imgSceneResizedHeight))
...
imgSceneAfterDetection = imgSceneResized.copy()
imgContours = np.zeros(imgSceneResized.shape, np.uint8)
imgContoursAfterSeveralCondition = np.zeros(imgSceneResized.shape, dtype = np.uint8)
...
aaaa = imgSceneResized.copy()
aaaaa = imgSceneResized.copy()
listOfImgPossiblePlate = []
numberOfPossiblePlate = 0
boundingRectInformation = []
img = imgSceneResized.copy()
for i in range(0, len(listOfContours)):
	...
	contourPerimeter = cv2.arcLength(contour, True)						 		 # perimeter of a closed contour
	...
	rectArea = boundingRectWidth * boundingRectHeight
	...
	print('ContourPerimeter: ' + str(contourPerimeter))
	...
	minOneHeightRatio = float(minOne)/(imgSceneResizedHeight)
	maxOne = max(minRectWidth, minRectHeight)
	maxOneWidthRatio = float(maxOne)/(imgSceneResizedWidth)
	ratio = float(maxOne)/minOne
	#minRectAreaAndImgSceneAreaRatio = float(minRectArea)/(imgSceneArea**(float(5)/4))
	print('ratio...............: ' + str(ratio))
	# 0.061538 0.075 0.00538461
	# 0.01 0.026 0.0004
	print('minOne: ' + str(minOne))
	print('maxOne: ' + str(maxOne))
	#print('maxOneWidthRatio: ' + str(maxOneWidthRatio))
	#print('minOneHeightRatio: ' + str(minOneHeightRatio))
	#print('minRectAreaAndImgSceneAreaRatio: ' + str(minRectAreaAndImgSceneAreaRatio))
	# tourFourRatio: loai bo nhung hinh khong giong chu nhat
	# minOne, maxOne: loai bo nhung hinh co do phan giai qua thap
	if twoFourRatio > 0.65 and minOneHeightRatio > 0.05 and maxOneWidthRatio > 0.05 and ratio > 1.0 and ratio < 2.0 and abs(abs(angle) - 45) >= 0:
		cv2.rectangle(imgSceneAfterDetection, (boundingRectX, boundingRectY), (boundingRectX + boundingRectWidth, boundingRectY + boundingRectHeight), (0, 0, 255), 2)
		cv2.imshow('imgSceneAfterDetection', imgSceneAfterDetection)

		boundingRectElement = [boundingRectX, boundingRectY, boundingRectWidth, boundingRectHeight]
		if do_contain_other_possible_plates(boundingRectElement, boundingRectInformation):
			print('vao day nay')
			continue
		numberOfPossiblePlate = numberOfPossiblePlate + 1
		print('Possible license plate number %d' % (numberOfPossiblePlate))
		boundingRectInformation.append(boundingRectElement)
		print('boundingRectInformation')
		print(len(boundingRectInformation))
		print(boundingRectInformation)
		
		deltaX = boundingRectWidth * 0.1
		deltaY = boundingRectHeight * 0.1
		possiblePlateX = int(boundingRectX - deltaX)
		possiblePlateY = int(boundingRectY - deltaY)
		print('possiblePlateY: ' + str(possiblePlateY))
		possiblePlateWidth = int(boundingRectWidth * 1.2)
		possiblePlateHeight = int(boundingRectHeight * 1.2)
		print('possiblePlateHeight: ' + str(possiblePlateHeight))
		print('imgSceneHeight: ' + str(imgSceneHeight))
		X = possiblePlateX + possiblePlateWidth
		Y = possiblePlateY + possiblePlateHeight
		if possiblePlateX < 0:
			possiblePlateX = 0
		if possiblePlateY < 0:
			possiblePlateY = 0
		...
		if Y > imgSceneHeight:
			print('y Vao day')
			Y = imgSceneHeight
		possiblePlateY = int(possiblePlateY * resizeRatio)
		Y = int(Y * resizeRatio)
		possiblePlateX = int(possiblePlateX * resizeRatio)
		X = int(X * resizeRatio)
		imgPossiblePlate = imgScene[possiblePlateY:Y,possiblePlateX:X]
		cv2.imshow('imgPossiblePlate', imgPossiblePlate)
		listOfImgPossiblePlate.append(imgPossiblePlate)
		#print('listOfImgPossiblePlate')
		#print(listOfImgPossiblePlate)
